<script>
  ...
  const examples = [
    { label: 'Age check',    code: 'let age = 22;\nlet canDrink = false;\n\nif (age >= 21) {\n  canDrink = true;\n}',                                                                                    cx: { time: 'O(1)', space: 'O(1)', timeWhy: 'O(1) — no matter how large your data grows, this code always does the same amount of work. Exactly 1 comparison (age >= 21). No loops, no recursion, no data-dependent branching.',                                                                                                          spaceWhy: 'O(1) — exactly 2 variables (age, canDrink) taking ~12 bytes total. No arrays, no objects, no dynamic allocation.' } },
    { label: 'Login gate',   code: 'let isLoggedIn = true;\nlet role = "admin";\nlet access = "denied";\n\nif (isLoggedIn && role === "admin") {\n  access = "granted";\n}',                           cx: { time: 'O(1)', space: 'O(1)', timeWhy: 'Still O(1) despite having 2 comparisons joined by &&. "Constant" means a fixed number of operations regardless of input size. The && operator also uses short-circuit evaluation.',                                                                                                                          spaceWhy: 'O(1) — exactly 3 variables. No new memory is allocated inside the if-block — we only reassign an existing variable.' } },
    { label: 'Temperature',  code: 'let temp = 38.5;\nlet status = "normal";\n\nif (temp > 37.5) {\n  status = "fever";\n}',                                                                           cx: { time: 'O(1)', space: 'O(1)', timeWhy: 'O(1) — one numeric comparison executes in a single CPU cycle. No loops or recursion — the execution path is always the same length.',                                                                                                                                                                                          spaceWhy: 'O(1) — 2 variables using fixed memory. The reassignment reuses the same variable slot — no new allocation.' } },
    { label: 'Discount',     code: 'let price = 100;\nlet isMember = true;\nlet discount = 0;\n\nif (isMember) {\n  discount = price * 0.2;\n  price = price - discount;\n}',                         cx: { time: 'O(1)', space: 'O(1)', timeWhy: 'O(1) — even with 2 statements inside the if-block, the total work is constant. O(1) means a bounded number of operations regardless of input values.',                                                                                                                          spaceWhy: 'O(1) — 3 variables allocated at the start. Inside the if-block, we compute new values into existing variables.' } },
    { label: 'if / else',    code: 'let score = 45;\nlet result = "";\n\nif (score >= 50) {\n  result = "pass";\n} else {\n  result = "fail";\n}',                                                     cx: { time: 'O(1)', space: 'O(1)', timeWhy: 'O(1) — exactly one branch executes, never both. The CPU evaluates the condition, then jumps to one branch. The skipped branch costs zero time.',                                                                                                                           spaceWhy: 'O(1) — 2 variables regardless of which branch runs. Both branches write to the same variable, so no extra memory is needed for else.' } },
  ];
